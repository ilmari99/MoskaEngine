...
def get_branched_model():
    ...
    img = tf.keras.layers.Dropout(rate=0.25)(img)
    img = tf.keras.layers.Dense(250,activation="relu")(img)
    img = tf.keras.layers.Dropout(rate=0.45)(img)
...
if __name__ == "__main__":
    all_dataset = create_tf_dataset(["./Dataset-Lumi2/Vectors/", "./Dataset-Lumi/Vectors/","./Dataset-Lumi3/Vectors/"],
    add_channel=False,
    get_part="full",
    )
    ...
    #model = get_loaded_model("./ModelBR3/model.h5")
    model = get_nn_model()
    print(model.summary())
    ...
    early_stopping_cb = tf.keras.callbacks.EarlyStopping(min_delta=0, patience=20, restore_best_weights=True, start_from_epoch=1)
